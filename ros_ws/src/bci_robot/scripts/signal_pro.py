...
class Signal_processer(object):
    ...
    def __init__(self, G=0.56, delta_f=0.46, alpha=0.99, bufferSize=32, fe = 256,
                speed = 0.1, aspeed = 0.5, thold = [3.5e-08, 2.1e-08, 1.4e-08]):
        '''Initializes the signal processor with the optimal parameters. The speed
        parameters were chosen as to run it safely in a small room.
        '''
        self.fe = fe
        ...
        #ROS members
        rospy.init_node('signal_processer', anonymous=True)
        self.sub = rospy.Subscriber("dataBCI", Float64, self.callback_data_listen)
        #TODO set it to String or something else
        #Is it better to publish directly to the robot or to send it to a different node?
    ...
    def callback_data_listen(self,sample):
        ...
        if self.i == self.N-1:
            ...
            self.i+=1
        #TEST------
        # self.pub.publish(Float64(data=self.y[0,0]))
    ...
    def apply_p_estimation(self):
        ...
        for j in [0,1,2]:
            self.buffer_z[j,self.i] = (1-self.alpha)*(self.y[j,0])**2 + self.alpha*(self.buffer_z[j,(self.i-1)%self.N])**2
